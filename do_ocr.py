...
# Create a temp folder in google drive to upload the files. You can delete
# this folder later.
...
for pdf_file in sorted(files):
    ...
    if not os.path.isfile(pdf_file.split('.')[0] + ".upload"):
        ...
        command = "gdput.py -t ocr -f   " + drive_folder_id + " " + \
            pdf_file + " | tee " + pdf_file.split('.')[0] + ".log"
...
logger.info(message)
command = "mv folder*.log currentfile.pdf  doc_data.txt pg*.pdf page* txt*   " + \
    '"' + temp_folder + '"'
logger.info("Running " + command.encode('utf-8'))
os.system(command.encode('utf-8'))
